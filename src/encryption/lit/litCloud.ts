...
// import { DecryptedFile, DecryptedString, EncryptFileParams, EncryptStringParams, EncryptedFile, EncryptedString, IOrbisEncryptionClient } from "../../types/encryption.js";
...
//     constructor(clientUrl: HttpsUrl) {
...
//     async authorize({ authenticator }: { authenticator: IOrbisAuth }): Promise<any> {
...
//             throw new OrbisError("[Encryption:Lit] Unsupported authentication method. Chain not supported " + chain, { supportedChains: this.supportedChains })
//         }
...
//             const hexSignature = toString(fromString(signature, "base58btc"), "base16")
...
//     async setSession({ user, session }: { user: AuthUserInformation; session: any; }): Promise<void> {
//         const _session = LitSession.fromSession(session)

//         if (_session.address.toLowerCase() !== user.metadata.address.toLowerCase()) {
//             this.clearSession()
//             throw new OrbisError("[Encryption:Lit] Session address mismatch", { session: _session, user })
//         }
...
//     // Strings
//     async encryptString({ string, encryptionRules }: EncryptStringParams): Promise<EncryptedString> {
//         await this.connect()
//         if (!this.session) {
//             throw new OrbisError("[Encryption:Lit] Not authorized. You need to call IEncryptionClient.authorize() or Orbis.login()")
//         }

//         const accessConditions: UnifiedAccessControlConditions = parseOrbisRules(encryptionRules)
...
//             throw new OrbisError("[Encryption:Lit] Unable to save encryption key. Encryption failed.")
//         }
...
//         const { encryptedContent, encryptionMetadata: { encryptedSymmetricKey, ...rules } } = encrypted
...
//             throw new OrbisError("[Encryption:Lit] Unable to get an encryption key.", { error: decryptionError })
//         }
...
//         if (err) {
//             throw new OrbisError("[Encryption:Lit] Unable to decrypt the string.", { error: err })
//         }
...
//     async encryptFile({ file, encryptionRules }: EncryptFileParams): Promise<EncryptedFile> {
//         await this.connect()
//         if (!this.session) {
//             throw new OrbisError("[Encryption:Lit] Not authorized. You need to call IEncryptionClient.authorize() or Orbis.login()")
//         }
...
//         const { encryptedContent, encryptionMetadata: { encryptedSymmetricKey, ...rules }, contentMetadata } = encrypted
...
//         return {
//             decryptedContent: new File([decryptedFile], contentMetadata.name, { type: contentMetadata.type })
//         }
//     }
// }
